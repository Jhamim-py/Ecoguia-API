...
async (req,res) => {
    ...
    if(!checkLength(image) | !checkLength(description)|| !checkLength(reference)){
     ...
     if( ! await checkArticle(title,reference)){
        console.log("Artigo já existente no banco de dados");
        return res.status(400).json({msg: "O artigo já exite no banco de dados"});
       }
        ...
        console.log(result)
        return res.status(200).json({msg:"Artigo criado com successo"});
      }
